...
assoc_address_list <-
  c(
    "/iepd:FoiaAnnualReport/foia:PendingPerfectedRequestsSection/foia:PendingPerfectedRequestsOrganizationAssociation"
  )

assoc_attrs_val_list <-
  c(
    "/iepd:FoiaAnnualReport/foia:PendingPerfectedRequestsSection/foia:PendingPerfectedRequestsOrganizationAssociation/foia:ComponentDataReference/@s:ref"
  )

assoc_attrs_org_list <-
  c(
    "/iepd:FoiaAnnualReport/foia:PendingPerfectedRequestsSection/foia:PendingPerfectedRequestsOrganizationAssociation/nc:OrganizationReference/@s:ref"
  )
```
...
section_values_function <- function(ORG_want,
  ...
  for (i in 1:nrow(assoc_want)){
    ...
    # Initialize a dataframe to store values from this node_section_subset_x  
    node_dataframe <- data.frame()
    
    # Initialize lists to store node_section_subset_x names and values 
    node_names <- list()
    node_values <- list()
    
    # Loop through each layer of a node_section_subset_x
    for (j in 1:xmlSize(node_section_subset_x)) {
      for (k in 1:length(names(xmlChildren(node_section_subset_x[[j]])))) {
        for (l in 1:length(names(xmlChildren(node_section_subset_x[[j]][[k]])))) {
          if (xmlName(node_section_subset_x[[j]][[k]][[l]]) == "text") {
            # If the element is "text", we've reached the lowest level. Go back up
            # one level and use that element name.
            node_names <-
              append(node_names, xmlName(node_section_subset_x[[j]][[k]]))
            node_values <-
              append(node_values, xmlValue(node_section_subset_x[[j]][[k]][[l]]))
          } else{
            # If we haven't reached the lowest level, use this element name.
            node_names <-
              append(node_names, paste0(xmlName(node_section_subset_x[[j]][[k]]),
                                        xmlName(node_section_subset_x[[j]][[k]][[l]])))
            node_values <-
              append(node_values, xmlValue(node_section_subset_x[[j]][[k]][[l]])) 
          }
        }
    ...
    # Create a dataframe for the current node_section_subset_x
    node_dataframe <- data.frame(
      t(unlist(node_values))
    )
    
    # Rename columns using values from node_names
    colnames(node_dataframe) <- unlist(node_names)
    
    node_dataframe$ComponentDataReference <- assoc_want$ComponentDataReference[i]
    node_dataframe$OrganizationReference <- assoc_want$OrganizationReference[i]
    
    section_values_year_x <- bind_rows(section_values_year_x, node_dataframe)
...
```{r}
# Check the name and values associated with a section of the XML.
xml_hhs <- xmlParse("./FOIA Sample/HHS_fy22.xml")
  
# Get the top-level node in the XML document we just parsed. This will allow
# us to more easily locate nodes of interest.
xml_top <- xmlRoot(xml_hhs)

xmlName(xml_top[["PendingPerfectedRequestsSection"]][["PendingPerfectedRequests"]][["SimplePendingRequestStatistics"]][["PendingRequestQuantity"]])

xmlValue(xml_top[["PendingPerfectedRequestsSection"]][["PendingPerfectedRequests"]][["SimplePendingRequestStatistics"]][["PendingRequestQuantity"]])
xmlValue(xml_top[["PendingPerfectedRequestsSection"]][["PendingPerfectedRequests"]][["SimplePendingRequestStatistics"]][["PendingRequestMedianDaysValue"]])
xmlValue(xml_top[["PendingPerfectedRequestsSection"]][["PendingPerfectedRequests"]][["SimplePendingRequestStatistics"]][["PendingRequestAverageDaysValue"]])

# Look at differences between files
diffr("FOIA Data Analysis KGM_v5.Rmd", "FOIA Data Analysis KGM_v2.Rmd")


# Check for missing columns between data frames and add the missing columns.
missing_columns2 <- dplyr::setdiff(colnames(req_disposition_10), colnames(req_disposition_15))
for (column in missing_columns) {
  req_disposition_15[[column]] <- NA
}

# Make sure columns are in the same order
col_order <- colnames(req_disposition_10)
req_disposition_15 <- req_disposition_15[col_order]

# Combine the data frames using rbind
combined_df <- rbind(req_disposition_10, req_disposition_15)

HHS_xmls <- xmlParse("HHS_xmls/HHS_fy15.xml")
                  
xml_top <- xmlRoot(HHS_xmls)

xmlValue(xml_top[['BacklogSection']][["Backlog"]])

# Find the names within a section.
names_list <- names(xmlChildren(xml_top[["RequestDispositionSection"]][["RequestDisposition"]]))
