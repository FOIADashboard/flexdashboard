...
# if a package doesn't exist, install the package
...
agencies <- c("DHS", "EPA", "DOL", "DOJ", "DOS", "HHS", "DOI",
              "GSA", "SEC", "VA")

# Agencies with budget data. Each .csv file should
# represent an agency. The .csv files are saved in data/csv.
# the .csv should be saved as {agency}_budget_ratio.csv
agencies_budget <- c("DHS", "DOL", "DOJ", "HHS", "DOS")

# A list of components for each agency. 
component_list <- list(
  "DHS" = c("CBP", "PRIV", "USCIS", "ICE", "OIG", "CRCL", "FPS",
            "FEMA", "FLETC", "I&A", "TSA", "USCG", "USSS"),
  "EPA" = c("HQ", "Region 1", "Region 2", "Region 3",
            "Region 4", "Region 5", "Region 6", "Region 7",
            "Region 8", "Region 9", "Region 10"),
  "DOL" = c("OSHA", "WHD", "OSEC", "OIG", "SOL", "OPA", "OCIA", "ETA", "OASP"),
  "DOJ" = c("EOIR", "OIG", "BOP", "DEA", "Civil Rights", "FBI", "OIP"),
  "HHS" = c("CDC", "ACF", "OIG", "CMS", "FDA", "NIH", "OS"),
  "DOS" = c("OIG", "DoS")
)
...
```{r setup-foia-data}
# functions to load the data. The data should be saved in the data/rds and
# data/csv folder for FOIA and budget data, respectively.
# Few things happen in the load functions. It reads the rds and csv files for the agencies
...
# get all available components for agency if it was not specified in component_list
...
```{r}
# capture section, agency, component,
# fiscal year, and variable selection
section <- reactive({
  input$section_input
})
agency <- reactive({
...
radioButtons(
  "plot_input_b",
  label = "Plot",
  choices = c("Budget", "Budget vs Backlog", "Public Affairs vs Budget"),
  selected = "Budget")
...
```{r, results='asis'}
# capture agency, component, and fiscal year selection
agency_b <- reactive({
...
# column name processing for budgets
...
# column name processing for budgets ratio
...
# column name processing for backlog
...
# process the fiscal year to match the budget data format
agencyBudget_year <- reactive({
  paste0("20", fy_b())
})

# plot for budget plot
...
# plot for budget vs backlog comparison plot
budget_v_backlot_plot <- reactive({
...
renderPlotly({
  if (input$plot_input_b == "Budget") {
    budget_plot()
  } else if (input$plot_input_b == "Budget vs Backlog") {
    budget_v_backlot_plot()
  } else {
