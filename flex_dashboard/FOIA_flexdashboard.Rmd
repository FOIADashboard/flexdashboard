...
master_foia_data <- load_all_agencies(agencies, data_dir = rds_dir)
...
ui_function(
  ...
  sidebar_text_list = sidebar_text_list_1,
  agencies = agencies)
```
...
personnel_filtered_data <- reactive({
  ...
  master_foia_data[[personnel_input_values$agency]][["PersonnelAndCostSection"]]
})
...
backlog_filtered_data <- reactive({
  ...
  master_foia_data[[backlog_input_values$agency]][["BacklogSection"]]
})
...
expedited_filtered_data <- reactive({
  ...
  master_foia_data[[expedited_input_values$agency]][["ExpeditedProcessingSection"]]
})
...
PR_filtered_data <- reactive({
  ...
  master_foia_data[[pr_input_values$agency]][["ProcessedRequestSection"]]
})
...
pendingR_column_select_ui <- selectInput(
  ...
  choices = c("Simple Pending Requests" = "SimplePendingRequestStatistics_PendingRequestQuantity",
...
pendingR_filtered_data <- reactive({
  ...
  master_foia_data[[pendingR_input_values$agency]][["PendingPerfectedRequestsSection"]]
})
...
filtered_input_manual_title_pendingR <- reactive({
  if (pendingR_input_values$data == "SimplePendingRequestStatistics_PendingRequestQuantity") {
    return("Simple Pending Request Quantity")
  } else if (pendingR_input_values$data == "SimplePendingRequestStatistics_PendingRequestMedianDaysValue") {
    return("Simple Pending Request Median Days")
  } else if (pendingR_input_values$data == "SimplePendingRequestStatistics_PendingRequestAverageDaysValue") {
    return("Simple Pending Request Average Days")
  } else if (pendingR_input_values$data == "ComplexPendingRequestStatistics_PendingRequestQuantity") {
    return("Complex Pending Request Quantity")
  } else if (pendingR_input_values$data == "ComplexPendingRequestStatistics_PendingRequestMedianDaysValue") {
    return("Complex Pending Request Median Days")
  } else if (pendingR_input_values$data == "ComplexPendingRequestStatistics_PendingRequestAverageDaysValue") {
    return("Complex Pending Request Average Days")
  } else if (pendingR_input_values$data == "ExpeditedPendingRequestStatistics_PendingRequestQuantity") {
    return("Expedited Pending Request Quantity")
  } else if (pendingR_input_values$data == "ExpeditedPendingRequestStatistics_PendingRequestMedianDaysValue") {
    return("Expedited Pending Request Median Days")
  } else if (pendingR_input_values$data == "ExpeditedPendingRequestStatistics_PendingRequestAverageDaysValue") {
...
appealDisp_filtered_data <- reactive({
  ...
  master_foia_data[[appealDisp_input_values$agency]][["AppealDispositionSection"]]
})
...
ui_function(
  ...
  sidebar_text_list = sidebar_text_list_budgets,
...
filtered_data_budget <- reactive({
  ...
  #   data_dir = csv_dir
  # )
  master_budget_data[[agencyBudget_input_values$agency]]
})
...
filtered_data_budget_compare <- reactive({
  ...
  #   data_dir = csv_dir
  # )
  master_budget_data[[BB_input_values$agency]]
})
...
renderPlotly({
  # temporary solution to missing budget column name (some missing underscore)
  if (!(filtered_input_budget_compare() %in% colnames(filtered_data_budget_compare()))) {
    plot_colname_budget_compare <- str_replace(filtered_input_budget_compare(), "_", ".")
  } else {
    plot_colname_budget_compare <- filtered_input_budget_compare()
  }
  
  if (plot_colname_budget_compare %in% colnames(filtered_data_budget_compare())){
    plot_single_column_budget(
      filtered_data_budget_compare(),
      plot_colname_budget_compare,
      BB_year()
    )
  } else {
    warning(str_glue("Column {plot_colname_budget_compare} not found in dataset."))
    return(NULL)
  }
})
```
...
renderPlotly({
  # temporary solution to missing budget column name (some missing underscore)
  if (!(filtered_input_budget_compare_backlog() %in% colnames(filtered_data_budget_compare()))) {
    plot_colname_budget_compare_backlog <- str_replace(filtered_input_budget_compare_backlog(), "_", ".")
  } else {
    plot_colname_budget_compare_backlog <- filtered_input_budget_compare_backlog()
  }
  
  if (plot_colname_budget_compare_backlog %in% colnames(filtered_data_budget_compare())){
    plot_single_column_backlog(
      filtered_data_budget_compare(), 
      plot_colname_budget_compare_backlog,
      BB_year()
    )
  } else {
    warning(str_glue("Column {plot_colname_budget_compare_backlog} not found in dataset."))
    return(NULL)
  }
})
```
